...
def show_points_known(image, belly_points, preds=None):
    ...
    plt.savefig('Belly_Rectified_Noise/' + str(i) + '/' + str(j) + '_coords.png', bbox_inches = 'tight', pad_inches = 0)  # Save the figure
    # plt.show()
...
def rectification(image, coords):
    ...
    plt.axis('off')
    # plt.savefig('Belly_Rectified_Smooth_Noise2/' + str(i) + '/' + str(j) + '_belly.png', bbox_inches = 'tight', pad_inches = 0)  # Save the figure
    # plt.show()
    io.imsave('Belly_Rectified_Noise/' + str(i) + '/' + str(j) + '_belly.png', sampled_img)
    plt.pause(0.05)
...
for i, sample in enumerate(saladataset):
    ...
    # Creating noisy belly points
    noisy_belly_points = []
    noisy_coords = []
    for j in range(10):
        noisy_belly_points.append(
            belly_points + np.random.normal(0, 5, belly_points.shape))  # Adding noise to belly_points
        show_points_known(image, noisy_belly_points[j])
        noisy_coords.append(getEquidistantPoints(noisy_belly_points[j], 500))
        rectification(image, noisy_coords[j])
